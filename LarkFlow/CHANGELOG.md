...
### Overview
取消 Phase 2 路由命中的 Top 5 截断。Agent 现在按 `weight` 降序读取**全部**命中的 `skills/*.md`，排序只影响"谁先进入上下文"（framework 1.3 → domain 1.2 → 其他 1.0），不再淘汰低权重命中。

### Changed
- **`agents/phase2_coding.md`**：第 2 步"Consult the Rules & Skills"从"取前 5"改为"读取全部命中"，`weight` 降序语义明确为"决定进入上下文的顺序"而非"决定是否读取"。
- **`README.md`**：路由表章节与"按需检索 (RAG) 知识库"章节同步更新文案，去掉 Top 5 表述。

### Motivation
当前 21 条 route / 21 份 skill，典型需求命中 3–8 条，Top 5 实际很少触发；一旦触发就意味着低权重的 `lang/*` 或 `governance/*` 规范被静默丢弃，反而隐藏上下文缺失。放开后 Phase 4 Reviewer 重跑的"matched set 对比"直接退化为全集对全集，审计路径更干净。

### Compatibility
- `rules/skill-routing.yaml` 数据不变；`defaults` 必读机制不变。
- `tests/prompts/` 现有断言为"应命中 skill 全部出现"的白名单形式，放开 Top 5 只会让断言更易通过，无需改动 fixture。
- Phase 1 设计阶段行为不变（本来就是列出完整命中集合）。
