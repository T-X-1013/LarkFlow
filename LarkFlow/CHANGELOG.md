...
- **契约软兼容**：
  ...
  - `MetricsItem.by_role: List[RoleMetrics]`：feature_multi 模板下暴露三路 tokens / duration，仪表盘 v2 可直接按 role 拆饼图
...
### Migration Notes
- **启用并行 review**：创建 pipeline 时传 `template="feature_multi"`（REST: `POST /pipelines {"requirement": "...", "template": "feature_multi"}`），或在前端 Pipelines 页下拉选择（下拉选项需在 `frontend/` 手工追加一条，已提交契约但未改前端）。
