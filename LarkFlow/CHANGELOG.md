...
### Overview
D1–D3 主干闭环：pipeline 从"黑箱自动跑"升级为"可操控流水线"。新增契约化的 REST API + Swagger、YAML 驱动的 DAG、协作式 pause/resume/stop 生命周期、第 2 个 HITL（部署审批）。Docker 入口统一切到双入口 `pipeline.app`（asyncio 并行 WS 长连 + FastAPI HTTP Server）。对应冲刺计划 V3.1 Must-have §1–§6。
...
### Overview
端到端重构"从自然语言需求到 AI 执行"的输入侧：新增 Phase 0 需求规范化阶段、Skill 路由从 Agent 自选改为代码决定、新增 Skill 闸门硬约束、前端增加两张可视化卡片。目的是把"输入不清 / AI 偷懒 / 漏读关键规范"三类问题从体系层面消除。

### Added
- **Phase 0 需求规范化**（`pipeline/phase0/`）：
  - `schema.py` — `NormalizedDemand` dataclass：goal / apis / persistence / nfr / domain_tags / open_questions / confidence / source
  - `normalizer.py` — 规则版抽取（正则 + 关键词，零 LLM 依赖，中英文双语）
  - `llm_classifier.py` — 可选 LLM 分类器（OpenAI 兼容，JSON schema 约束，失败自动降级规则版，与规则版做 hybrid 合并）
  - `_inject_normalized_demand` engine helper：Phase1/2/4 的 system prompt 尾部都注入 `## Normalized Demand (authoritative)` 段
- **Skill 路由三层化**（`pipeline/skills/router.py` + `rules/skill-routing.yaml` v2 schema）：
  - Tier-0 `baseline`（kratos/error/observability/logging 4 条无条件必读）
  - Tier-1 `conditional`（6 条，关键词触发器命中即必读，不受 top-K 截断）
  - Tier-2 `routes`（关键词权重召回 top-K=5）
  - `route_from_text()` 纯代码决定，结果落 `session["skill_routing"]`；agent 不再自己解析 YAML
- **Embedding 语义召回**（`pipeline/skills/semantic.py`）：Tier-2 可选第二通道；skill 语料做一次 embedding 落 `telemetry/skill-embeddings.json`；cosine ≥ 阈值纳入候选；与关键词通道并集，`MatchReason.source = keyword | semantic | both`
- **Skill 闸门**（`pipeline/skills/gate.py` + `pipeline/llm/tools_runtime.py`）：
  - `file_editor` read 成功时工具层把相对路径写入 `session["skills_read"]`
  - Phase 2 结束前 `check_coverage`：baseline/conditional 未读 → 追加一轮 user message 强制补读；超 `LARKFLOW_SKILL_GATE_MAX_RETRIES` 仍缺 → `StageStatus.FAILED`
  - Tier-2 漏读不 block，只标 optional
- **Phase 0 澄清回路**：
  - `PipelineStatus.WAITING_CLARIFICATION` + `CheckpointName.CLARIFICATION` + `PHASE_CLARIFICATION_PENDING`
  - 任一 `blocking=True` 问题或置信度低于 floor 自动挂起，不进 Phase 1
  - REST `POST /pipelines/{id}/clarify` 接收答案 → 合入 raw_demand → 重规范化 → 仍 blocking 则继续挂起，否则进 Phase 1
- **REST 契约扩展**：`PipelineState` 新增 `normalized_demand / skill_routing / skill_gate` 三个可选字段；新增 `ClarificationRequest / ClarificationAnswerItem`
- **前端两张卡片**：
  - `pages/NormalizedDemandCard.tsx` — 结构化需求展示；`waiting_clarification` 时自动变成问答表单（blocking 行标红，提交后热刷新）
  - `pages/SkillRoutingCard.tsx` — 路由命中清单；分三 tier 颜色徽标；Source 列 keyword/semantic/both；闸门通过/未通过徽标 + ✓已读/✗未读列（mandatory 未读标红）
  - `lib/api.ts` 新增 `clarifyPipeline`；MSW handler + fixture 完整支持本地 demo
- **测试**：新增 62 用例覆盖 router / semantic / gate / phase0 normalizer / phase0 llm / engine 注入 / 澄清状态机；总数 268 → 386

### Changed
- `pipeline/core/contracts.py` — 新增 `SkillRoutingReason / SkillRoutingSnapshot / SkillGateSnapshot / NormalizedDemandSnapshot` 及其嵌套类型
- `pipeline/core/engine.py` — `start_new_demand` 在启动即计算并落盘 normalized_demand + skill_routing；`_run_phase` 注入两段权威 prompt；新增 `_enforce_skill_gate / _needs_clarification / resume_from_clarification`
- `pipeline/core/engine_control.py` — `build_state` 反序化三个新字段；`_infer_status` 识别 `clarification_pending`
- `rules/flow-rule.md` Meta-Rule 改成"路由由 pipeline 预先算好，agent 不再自己解析 YAML"
- `agents/phase1_design.md` / `phase2_coding.md` / `phase4_review.md` — 以注入段为权威清单；Phase1 设计文档的 `## Relevant Skills` 要求原样抄写，不能自行增删
- `scripts/gen_skill_routing_md.py` — 适配新 schema（baseline/conditional/routes 三段镜像）

### Env (new)
- `LARKFLOW_PHASE0_LLM_ENABLED` / `LARKFLOW_PHASE0_CONFIDENCE_FLOOR` / `LARKFLOW_PHASE0_MODEL` / `LARKFLOW_PHASE0_API_KEY` / `LARKFLOW_PHASE0_BASE_URL`
- `LARKFLOW_SEMANTIC_ROUTER_ENABLED` / `LARKFLOW_SEMANTIC_ROUTER_THRESHOLD` / `LARKFLOW_EMBEDDING_API_KEY` / `LARKFLOW_EMBEDDING_BASE_URL` / `LARKFLOW_EMBEDDING_MODEL`
- `LARKFLOW_SKILL_GATE_ENABLED` / `LARKFLOW_SKILL_GATE_MAX_RETRIES`

默认配置下（全部关闭/默认值）行为与 v1.9.1 一致，零回归；开启后逐项叠加生效。完整列表见 `.env.example`。

### Migration Notes
- **老 demand 零影响**：所有新字段在 session 缺失时前端返回 null、后端安全降级，渲染路径原样兼容。
- **LLM 分类器需显式启用**：`LARKFLOW_PHASE0_LLM_ENABLED=1` 且 `OPENAI_API_KEY` 可用；不配置则走规则版，与 v1.9.1 流程一致。
- **Semantic router 首次冷启动**：开启后第一次需求会对全部 skill 做 embedding 落盘（~10s），之后命中缓存。
- **闸门默认开启**：Phase 2 漏读 baseline/conditional 会强制补读；想关闭验证差异设 `LARKFLOW_SKILL_GATE_ENABLED=0`。

### Validation
- 386 unit passed（原 268 + 新增 62 + 既有扩展 56）
- 前端 `tsc -b && npm run build` 全绿
- `.env.example` 模板与 `.env` 开关均已校验可被代码读取（`is_enabled()` 返回 True）
