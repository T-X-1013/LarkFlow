...
from pipeline.config import phases as phases_config
...
# .env 的加载由 `pipeline.config` 统一负责（`from pipeline.config import lark ...`
# 已把 load_dotenv 触发在首次 import）。本文件不再单独调用。
...
def _env_int(name: str, default: int) -> int:
    ...
    @return:
        返回不小于 1 的整数配置
    """
    return runtime_config.env_positive_int(name, default)
...
def _prepare_tech_doc(
    ...
    approve_target = lark_config.demand_approve_target()
    approve_type = lark_config.demand_approve_receive_id_type()
...
# 每个阶段的 prompt / kickoff / banner 配置从 `LarkFlow/config/phases.yaml` 读取，
# 保持和代码解耦；文案调整不再需要改 .py。原字典语义通过下面的 loader 复原。
_PHASE_CONFIG: Dict[str, Dict[str, Any]] = phases_config.load_phase_config()
...
# 数据来自 `config/phases.yaml`。
_PHASE_BANNER: Dict[str, tuple] = phases_config.load_phase_banner()
