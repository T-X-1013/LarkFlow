...
- **Provider 运行时切换已接通**：`pipeline/llm_adapter.py` 已把 Anthropic / OpenAI / Qwen / Doubao 的 client 构建、turn 归一和 session mode 收口到 provider registry；`PUT /pipelines/{id}/provider` 现在不仅修改状态，还会真实影响后续 `start` 时使用的 provider。为避免不同 provider 的会话协议不兼容，切换只允许发生在 `create` 后、`start` 前；运行中的 pipeline 再切换会返回冲突错误。
- **LLM 结构化埋点 + metrics 聚合已接通**：`pipeline/observability.py` 与 `llm_adapter.py` 统一输出 `provider / model / tokens_input / tokens_output / duration_ms / retry` 等字段；`GET /metrics/pipelines` 已能基于 `session["metrics"]` 返回真实 token 与耗时聚合，`GET /pipelines/{id}` 在阶段结果落盘后可返回 `stages.design / coding / test / review` 的结构化快照。
- **前端控制台骨架**：`LarkFlow/frontend/` 基于 `Vite + React + TypeScript + MSW` 提供首页、Pipeline 列表页、详情页和仪表盘；D5 已接通真实 API，开发态仍可通过 `MSW` 回退到 mock 数据源。
...
│   ├── frontend/
│   │   ├── src/
│   │   │   ├── lib/api.ts            # 前端 API 抽象层；支持 live API / MSW 双模式
...
`LarkFlow/frontend/` 当前已经支持真实 API 联调与 `MSW` mock 双模式运行。

- live API 模式：列表页与仪表盘先读取 `GET /metrics/pipelines`，再按 pipeline id 调 `GET /pipelines/{id}` 补全详情
- mock 模式：开发态可通过 `MSW` 回退到本地 mock 数据源，便于脱离后端独立演示页面结构与交互

默认行为：

- 若显式设置 `VITE_USE_MSW=1`，前端强制使用 mock
- 若显式设置 `VITE_USE_MSW=0` 且配置 `VITE_API_BASE_URL`，前端使用真实后端
- 若开发态未配置 `VITE_API_BASE_URL`，前端默认启用 `MSW`
