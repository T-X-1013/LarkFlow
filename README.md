...
- **契约化 REST API + Swagger**：9 个端点覆盖 pipeline 全生命周期（create / start / pause / resume / stop / get / artifact / checkpoint approve・reject / provider 切换 / metrics），`/docs` 即 Swagger UI。
- **可配置 DAG**：四阶段流转由 `pipeline/dag/default.yaml` 描述，替换原硬编码序列；后续扩 bugfix / feature / refactor 模板只改 YAML。
...
- **Provider Registry + reload 基础**：`pipeline/llm_adapter.py` 已把 Anthropic / OpenAI / Qwen / Doubao 的 client 构建、turn 归一和 session mode 收口到 provider registry，为后续 `set_provider` 运行时切换和无重启 reload 提供库层基础。
- **LLM 结构化埋点**：`pipeline/observability.py` 与 `llm_adapter.py` 已统一输出 `provider / model / tokens_input / tokens_output / duration_ms / retry` 等字段，便于前端 metrics、日志检索和后续 OTEL 聚合。
- **前端控制台骨架**：`LarkFlow/frontend/` 基于 `Vite + React + TypeScript + MSW` 提供首页、Pipeline 列表页、详情页和仪表盘；D1-D3 阶段先全部走 mock 契约，后续只替换数据源。
...
graph TD
    ...
    subgraph lfe["LarkFlow 核心引擎 (Python Pipeline)"]
        APP["app.py<br/>asyncio 并行 WS + FastAPI"]
        API["api/routes.py<br/>§八 9 端点 + /docs"]
        EAPI[engine_api.py<br/>9 方法 facade]
        ECTL[engine_control.py<br/>cancel/pause/resume 注册表]
        DAG[(dag/default.yaml<br/>YAML 驱动)]
        ...
        TD --> DOC
        TD --> Q
        Q -.->|approve / reject<br/>（或 POST /checkpoints/design/approve）| WS
        ...
        D4 -->|_request_deploy_approval| QD
        QD -.->|approve / reject<br/>（或 POST /checkpoints/deploy/approve）| WS
        WS -->|trigger_deploy| DP
...
│   │   ├── observability.py
│   │   ├── lark_client.py
│   │   ├── lark_cards.py             # 设计 / 部署两张审批卡模板（双 HITL）
│   │   ├── lark_doc_client.py
│   │   ├── lark_interaction.py
...
│   ├── frontend/
│   │   ├── src/
│   │   │   ├── lib/api.ts            # 前端 API 抽象层；当前默认走 MSW mock
...
- `LarkFlow/pipeline/observability.py` 给每个需求发结构化 JSON 日志（stdout + `logs/larkflow.jsonl`），读 `AgentTurn.usage` 累加到 `session["metrics"]`，并统一输出 `provider / model / tokens_input / tokens_output / duration_ms / retry` 等字段，可用 `jq` 按 `demand_id` 聚合 token、延迟与失败重试信息。
...
同时，`llm_adapter.py` 内部已经把 provider 选择从散落的 `if/else` 分发改为 registry 机制，统一管理：
...
这让后续 `engine_api.set_provider(...)` 的真实接线、provider 动态扩展以及无重启 reload 都有了稳定的库层落点。

### 5. 前端控制台

`LarkFlow/frontend/` 是 D1-D3 并行开发出来的控制台壳，当前定位是“按冻结契约跑通可交互 mock”，而不是立即绑定真实后端：

- 技术栈：`Vite + React + TypeScript + MSW`
- 页面：`/` 首页、`/pipelines` 列表页、`/pipelines/:pipelineId` 详情页、`/dashboard` 仪表盘
- 契约来源：`pipeline/contracts.py` + `pipeline/api/routes.py`
- 当前能力：创建 mock pipeline、状态切换、provider 切换、checkpoint approve/reject、artifact 预览、metrics 展示
- 当前边界：D1-D3 默认仍走 `MSW`，真实 API 联调属于后续阶段
...
`LarkFlow/pipeline/contracts.py` 是前端、REST、WS 回调共用的**单一事实来源**：
...
`LarkFlow/pipeline/engine_api.py` 是 9 个方法的 facade，被 REST 路由与 WS 卡片回调共用，两条入口殊途同归：
...
| `list_states` | `GET /metrics/pipelines` | — |
