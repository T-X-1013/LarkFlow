...
- **可配置 DAG**：流转由 `pipeline/dag/*.yaml` 描述，当前已内置 6 套模板：`default / feature / bugfix / refactor / feature_multi`（四阶段 0-1 路径）+ `brownfield`（五阶段存量改造路径，多一个 Phase 0 Inventory 前置节点）。除阶段顺序外，还可按模板声明 Review 回归目标（feature → coding；brownfield → inventory）和是否挂 deploy checkpoint。
- **Brownfield（存量改造）链路**：当 `demo-app/` 已存在 `go.mod` 时，引擎自动探测为 brownfield 模式，REST `POST /pipelines` 在 `template="default"` 时会切到 `brownfield` 模板：先跑 Phase 0 Inventory（`agents/phase0_inventory.md`，使用 `grep_symbol` / `list_dir_summary` 两个轻量索引工具扫描存量代码并产出结构化 `code_map`），再把 `<code-map>` 块注入 Phase 1 Design 与 Phase 4 Review 的 system prompt；Design 必须填 `## Existing Surface Touched` / `## Compatibility Risks` 两节，Review 阶段对此做 🔴 预飞校验，缺失即 REGRESS 回到 Inventory 重新盘点（最多 3 次），与 0-1 路径不冲突。
...
- **前端控制台**：`LarkFlow/frontend/` 基于 `Vite + React + TypeScript + MSW` 提供首页、Pipeline 列表页、详情页和仪表盘；当前控制台已形成“Base 记录 + Pipeline 运行态 + Metrics”三层读模型：列表页与仪表盘以 `/demands` 为主数据源，详情页继续消费 `/pipelines/{id}` 与 artifact / checkpoint 系列接口，同时保留 MSW 兜底本地演示。
- **浏览器圈选改页面 MVP**：控制台已接入 `picker/` 能力，可在开发态持续圈选前端页面元素，依赖 `data-lark-src`、选区上下文和命名参照生成本地源码预览，并完成确认、回滚、交付摘要、提交前检查、准备提交与安全 commit。
- **飞书文档创建 Pipeline**：支持通过飞书文档链接（docx/wiki）创建 Pipeline，自动读取文档内容并在多维表格创建记录；Base 侧的 `状态 / 需求ID / 需求文档 / 技术方案文档 / 模板 / 触发时间` 字段会参与发卡、状态回写与前端展示。
...
│   ├── LarkFlow.md
│   ├── agents/
│   │   ├── phase0_inventory.md              # brownfield-only 前置：扫存量出 code_map JSON
│   │   ├── phase1_design.md                 # brownfield 下强制写 Existing Surface / Compat Risks
│   │   ├── phase2_coding.md
│   │   ├── phase3_test.md
│   │   ├── phase4_review.md                 # 单 Agent Review；含 brownfield 设计预飞 🔴
...
│   │   │   ├── refactor.yaml
│   │   │   ├── feature_multi.yaml
│   │   │   ├── brownfield.yaml       # 存量改造 5 阶段：inventory → design → coding → test → review
│   │   │   └── schema.py
│   │   ├── lark/                     # 飞书集成层
...
│   │   │   ├── doc_reader.py         # 飞书文档读取（从文档创建 Pipeline）
...
│   │   │   ├── tools_runtime.py      # inspect_db / file_editor / grep_symbol / list_dir_summary / run_bash 实现
...
`LarkFlow/agents/` 里定义了各阶段的 System Prompt：

- `phase0_inventory.md`：**brownfield-only 前置阶段**，专门扫描 demo-app 存量代码，输出结构化 `code_map` JSON（`existing_domains / existing_apis / existing_tables / naming_conventions / recommended_touch_points`），供 Phase 1 与 Phase 4 直接引用。工具预算硬限：`grep_symbol` ≤ 8 次，`list_dir_summary` ≤ 3 次，`run_bash` 禁用，避免上下文爆量。
- `phase1_design.md`：系统设计与审批前方案输出；brownfield 模式下强制新增 `## Existing Surface Touched` / `## Compatibility Risks` 两节，写空 / "none" / "n/a" 均会被 Phase 4 REGRESS。
- `phase2_coding.md`：按 `rules/` 和 `skills/` 实现 Go 代码。
- `phase3_test.md`：补测试并运行验证。
- `phase4_review.md`：单 Agent 模式下，从规范角度复查并修正问题（default / feature / bugfix / refactor / brownfield 模板使用）。**brownfield 路径下增加预飞校验**：在读代码之前先看设计文档两节是否填好，缺失即 `<review-verdict>REGRESS</review-verdict>` 回到 Phase 0 Inventory（不是 coding，brownfield 的根因常在"没看清存量"）。
...
目前这套控制台已经从“纯 mock 壳”推进到“真实后端 + MSW 兜底”的混合模式：

- Pipeline 列表页会优先拉取 `GET /demands`，直接展示飞书 Base 当前需求记录，再按需跳转到运行态详情。
- Dashboard 也以 `GET /demands` 为主数据源，只统计 Base 当前记录，避免历史本地 session 污染观测口径。
- 详情页的 start / pause / resume / stop / provider 切换 / checkpoint 操作仍按真实 `/pipelines` REST 契约接线。
...
# 从飞书文档创建 Pipeline
curl -X POST http://localhost:8000/pipelines/from-doc \
  -H 'Content-Type: application/json' \
  -d '{"doc_url":"https://your-tenant.feishu.cn/wiki/BHXSwvm4Fip1nZkExmucZakhn5f"}'
...
- 有 `模板` 列（可选；为空时回退 `default`，支持 `default / feature / bugfix / refactor / feature_multi`）
