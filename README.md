...
LarkFlow 是一个**完全无头（Headless）、基于多阶段多 Agent（串行）自动化研发工作流引擎**。

[![Version](https://img.shields.io/badge/version-2.3.0-blue.svg)](https://github.com/your-repo/larkflow)
...
- **可配置 DAG**：四阶段流转由 `pipeline/dag/*.yaml` 描述，当前已内置 `default / feature / bugfix / refactor/feature_multi` 五套模板；除基础阶段顺序外，还可按模板声明 Review 回归策略和是否挂 deploy checkpoint以及三路review。
